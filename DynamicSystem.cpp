...
namespace DynS
{
	...
	void DynamicSystem::AdaptiveExplicitRungeKuttaFourthOrder()
	{
		...
		if (abs(intactStepNorm - halfStepNorm) / abs(intactStepNorm) > 1e-2) {
			this->dt /= 2; // Error is too large; decrease step size.
			this->point_of_trajectory = halfStep;
		}
		else {
			doubleStepNorm = doubleStep.norm();
			if (abs(intactStepNorm - doubleStepNorm) / abs(intactStepNorm) < 1e-3) {
				this->dt *= 2; // Larger error is acceptable; increase step size.
				this->point_of_trajectory = doubleStep;
			}
			else this->point_of_trajectory = intactStep; // no adjusting needed
		}

...
public:
	...
	friend std::ostream& operator<< (std::ostream& in, matrix& matrix);
...
std::istream& operator>> (std::istream& in, matrix& matrix) {
	...
	std::cout << "width: ";
	std::cin >> matrix.width;
	std::cout << "height: ";
	std::cin >> matrix.height;
	delete[] matrix.values;
	matrix.values = new double[matrix.width*matrix.height];

	for (int l = 0; l < matrix.width*matrix.height; l++) {
		std::cout << (int)(l / matrix.width) + 1 << "|" << l - (((int)(l / matrix.width))*matrix.width) + 1 << " : ";
		in >> matrix.values[l];
	}
	std::cout << "---------------end insert matrix" << std::endl;
	return in;
}
